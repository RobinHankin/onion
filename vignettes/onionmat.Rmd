...
```{r}
A <- newonionmat(rquat(21),matrix(0,7,3))
A
```
...
```{r}
A <- newonionmat(rquat(21),matrix(0,3,7))
umbral <- state.abb[1:7]
rownames(A) <- letters[1:3]
colnames(A) <- umbral

B <- newonionmat(rquat(28),matrix(0,7,4))
rownames(B) <- umbral
...
```{r}
C <- newonionmat(rquat(14),matrix(0,7,2))
rownames(C) <- umbral
...
```{r}
x <- cprod(onionmat(roct(12),4,3))
y <- cprod(onionmat(roct(12),4,3))
```

then the Jordan identity in numerical form for these two matrices would be:
